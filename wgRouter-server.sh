...
EOF
    ...
    if [[ $custom_ip =~ ^[Yy]$ ]]; then
        ...
        # 添加独立SNAT规则
        rule_up="iptables -t nat -I POSTROUTING 1 -s $client_ip/24 -o $ext_if -j SNAT --to-source $client_nat_ip"
        rule_down="iptables -t nat -D POSTROUTING -s $client_ip/24 -o $ext_if -j SNAT --to-source $client_nat_ip"
...
delete_client() {
    ...
    # 删除关联的iptables规则
    ext_if=$(ip route show default | awk '/default/ {print $5; exit}')
    nat_ips=$(iptables-save -t nat | grep "SNAT --to-source" | grep " -s $client_ip/24 " | awk '{print $NF}')
    for nat_ip in $nat_ips; do
        iptables -t nat -D POSTROUTING -s "$client_ip/24" -o "$ext_if" -j SNAT --to-source "$nat_ip"
    done
