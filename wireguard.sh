...
generate_client_name() {
    while true; do
        ...
        random_str=$(head /dev/urandom | tr -dc 'A-Za-z0-9' | head -c 5)  # 从4改为5
        pid=$(echo $$ | cut -c 1-3)  # 加入进程ID片段
        base_unsanitized="clt-${timestamp}-${random_str}-${pid}"  # 组合更多唯一因子
...
set_client_name() {
    # 替换非法字符为下划线，并限制长度
    client=$(sed 's/[^A-Za-z0-9_-]/_/g' <<< "$unsanitized_client" | cut -c-20)  # 增加长度限制到20
    # 如果清理后为空，生成更安全的默认名称
    if [ -z "$client" ]; then
        client="client-$(date +%s | sha256sum | base64 | head -c 4)"  # 使用哈希片段
    fi
}
...
set_client_name() {
    # 替换非法字符为下划线，并限制长度
    client=$(sed 's/[^A-Za-z0-9_-]/_/g' <<< "$unsanitized_client" | cut -c-15)
    # 如果清理后为空，使用默认前缀
    if [ -z "$client" ]; then
        client="client-$(head /dev/urandom | tr -dc '0-9' | head -c 3)"
    fi
}
...
check_args() {
    if [ "$auto" != 0 ] && [ -e "$WG_CONF" ] && [ "$add_client" = 0 ]; then
...
Install options (optional):
  ...
  --port [number]                port for WireGuard (1-65535, default: 51628)
...
show_config() {
    if [ "$auto" != 0 ]; then
        ...
        [ -n "$server_port" ] && port_text="$server_port" || port_text=51628
...
select_port() {
    if [ "$auto" = 0 ]; then
        ...
        [ -n "$server_port" ] && port="$server_port" || port=51628
    fi
}
...
enter_first_client_name() {
    if [ "$auto" = 0 ]; then
        ...
        read -rp "Name [auto-generate]: " unsanitized_client
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 10.17.0.0/24 ! -d 10.17.0.0/24 -j MASQUERADE
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 10.17.0.0/24 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 10.17.0.0/24 ! -d 10.17.0.0/24 -j MASQUERADE
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 10.17.0.0/24 -j ACCEPT
...
EOF
    if [ "$export_to_home_dir" = 1 ]; then
        ...
        chown "$SUDO_USER:$SUDO_USER" "$export_dir$client".png 2>/dev/null
    fi
    chmod 600 "$export_dir$client".conf
    chmod 600 "$export_dir$client".png 2>/dev/null
}
...
update_sysctl() {
    ...
    # 添加BBR配置
    echo "net.core.default_qdisc=fq" >> "$conf_opt"   # 添加队列算法
    echo "net.ipv4.tcp_congestion_control=bbr" >> "$conf_opt"  # 启用BBR
    # 下载优化配置（如果可用）
...
show_client_qr_code() {
    ...
    qrencode -t PNG -s 6 -l H -o "$export_dir$client.png" < "$export_dir$client.conf"
    echo "QR code saved to: $export_dir$client.png"
}
